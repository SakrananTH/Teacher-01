...
do $$
begin
  alter table public.assignments add constraint assignments_max_score_check check (max_score is null or max_score >= 0);
exception
...
do $$
begin
  alter table public.assignment_submissions add constraint assignment_submissions_score_check check (score is null or score >= 0);
exception
  when duplicate_object then null;
end $$;